/* ═══════════════════════════════════════════════════════════════
   iLoveConvert — auth.js  v1.1
   FIXES v1.1:
     ✅ Welcome toast only shows ONCE per browser session
     ✅ Auto-logs every tool page visit to Supabase history
     ✅ Proper SIGNED_IN vs TOKEN_REFRESHED distinction
     ✅ IndexedDB fallback for logged-out users
     ✅ Local → cloud sync on login
   ═══════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  /* Pages that are NOT tool pages — skip auto-logging */
  var NON_TOOL_PAGES = [
    'index.html','pdf-tools.html','image-tools.html','video-tools.html',
    'qr-tools.html','developer-tools.html','login.html','dashboard.html',
    'history.html','account.html','about.html','contact.html',
...
.ilc-auth-btn:hover{background:rgba(229,62,62,.2);transform:translateY(-1px)}
...
.ilc-avatar-btn .ilc-online{position:absolute;bottom:-2px;right:-2px;width:9px;height:9px;
  border-radius:50%;background:#22c55e;border:2px solid #050508}
[data-theme="light"] .ilc-avatar-btn .ilc-online{border-color:#f7f8fc}
.ilc-drop{position:absolute;top:calc(100% + 8px);right:0;min-width:210px;
  background:var(--bg2,#0a0a10);border:1px solid rgba(255,255,255,.1);border-radius:14px;
  box-shadow:0 16px 50px rgba(0,0,0,.5);z-index:9999;overflow:hidden;display:none;
  animation:ilcDropIn .18s ease}
...
.ilc-logout-btn{color:#e53e3e!important}
.ilc-logout-btn:hover{background:rgba(229,62,62,.08)!important}
...
.ilc-fav-btn{display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:9px;
  ...
  font-size:.78rem;font-weight:700;color:var(--txt2,#8888a8);cursor:pointer;font-family:inherit;transition:.18s}
...
`;
  ...
  function isToolPage() {
    ...
    if (NON_TOOL_PAGES.indexOf(page) !== -1) return false;
    if (!page.endsWith('.html')) return false;
    return !!(document.querySelector('.tool-hero') || document.querySelector('.wrap'));
  }
  ...
  function getToolName() {
    ...
    if (h1) {
      ...
      return text.split('—')[0].split('–')[0].trim().substring(0, 60);
    }
    return document.title.split('|')[0].split('—')[0].trim().substring(0, 60);
  }

  /* ── AUTO-LOG TOOL VISIT ────────────────────────── */
  /* KEY FIX: called ONCE per page load, writes to Supabase directly */
  var _autoLogged = false;
  function autoLogToolVisit(userId) {
    if (_autoLogged) return;
    ...
    if (userId && window.ilcSupabase) {
      window.ilcSupabase.from('tool_history')
        .insert([{
          user_id    : userId,
        ...
        .then(function (r) {
          if (r.error) {
            console.warn('[iLC] History insert error:', r.error.message);
            idbSave(record); /* fallback to local */
          }
        })
        .catch(function () { idbSave(record); });
    } else {
  ...
  /* FIX: sessionStorage clears when all tabs are closed,
     so welcome shows only on first page after login, NOT on every navigation */
  function maybeShowWelcome(user) {
    if (sessionStorage.getItem('ilc_welcomed')) return;
    sessionStorage.setItem('ilc_welcomed', '1');
  ...
  function injectNavWidget() {
    ...
    if (iNavSp)        iNavSp.insertAdjacentElement('afterend', widget);
    else if (navSp)    navSp.insertAdjacentElement('afterend', widget);
    else if (catTheme) catTheme.insertAdjacentElement('beforebegin', widget);
    else {
  ...
  function injectMobAuth() {
    var mobTarget = document.getElementById('i-mob-nav') ||
                    document.getElementById('mobDrawer')  ||
                    document.getElementById('cat-mob-menu');
    if (!mobTarget) return;
  ...
  function updateNavUI(user) {
    ...
    var emailEl   = document.getElementById('ilcDropEmail');
  ...
  function guardPage(user) {
    var page = getCurrentPage();
    var isProtected = PROTECTED.some(function (p) { return page === p; });
    if (isProtected && !user) {
  ...
  window.ilcSignOut = function () {
    ...
    sessionStorage.removeItem('ilc_welcomed');
  ...
  function idbSave(record) {
    openIDB(function (db) {
      ...
      db.transaction(IDB_STORE, 'readwrite').objectStore(IDB_STORE).add(record);
    });
  }
  ...
  function syncLocalToCloud(userId) {
    idbGetAll(function (records) {
      ...
      window.ilcSupabase.from('tool_history').insert(rows).then(function (res) {
        if (!res.error) {
          idbClear();
          toast('☁️ ' + rows.length + ' records synced to cloud!');
        }
      });
    });
  }
    ...
    if (user && window.ilcSupabase) {
      window.ilcSupabase.from('tool_history')
        .insert([{ user_id: user.id,
                   tool_name: record.tool_name, file_name: record.file_name,
                   action_type: record.action_type, result_url: record.result_url,
                   created_at: record.created_at }])
        .then(function (r) { if (r.error) idbSave(record); });
    } else {
      idbSave(record);
    }
  };
  ...
  function loadFavs(userId) {
    if (!window.ilcSupabase) return;
    window.ilcSupabase.from('favorite_tools').select('tool_name').eq('user_id', userId)
      .then(function (r) {
        if (!r.error && r.data) {
  ...
  window.ilcToggleFav = function (toolName) {
    ...
    if (idx === -1) {
      ...
      window.ilcSupabase.from('favorite_tools')
        ...
        .then(function (r) {
          if (!r.error) { window._ilcFavs.splice(idx, 1); updateFavBtn(); toast('✅ Removed from favourites'); }
        });
    }
  };

  function injectFavBtn() {
    if (document.getElementById('ilcFavBtn')) return;
  ...
  function updateFavBtn() {
    ...
    if (!btn) return;
    var isFav = window._ilcFavs.indexOf(getToolName()) !== -1;
  ...
  function boot() {
    ...
    if (!window.ilcSupabase) {
      updateNavUI(null);
      autoLogToolVisit(null);
      return;
    }

    /* KEY FIX: getSession() runs first — gives us persisted user immediately.
       onAuthStateChange fires on token refresh too, so we distinguish events
       to avoid showing welcome toast on every page. */
    window.ilcSupabase.auth.getSession().then(function (r) {
      ...
      if (user) {
        loadFavs(user.id);
        maybeShowWelcome(user);   /* ← only shows once per browser session */
        autoLogToolVisit(user.id); /* ← log this tool page visit */
      } else {
        autoLogToolVisit(null);   /* ← log anonymously to IndexedDB */
      }
    }).catch(function () {
      updateNavUI(null);
      guardPage(null);
    ...
    /* Auth state listener — handles logout, token refresh */
    window.ilcSupabase.auth.onAuthStateChange(function (event, session) {
      ...
      if (event === 'SIGNED_IN') {
        /* Only fires on actual new sign-in, not on token refresh */
        guardPage(user);
        syncLocalToCloud(user.id);
        loadFavs(user.id);
        /* Don't call maybeShowWelcome here — getSession() above handles it */
        autoLogToolVisit(user.id);
      }
      if (event === 'SIGNED_OUT') {
        window._ilcFavs = [];
        sessionStorage.removeItem('ilc_welcomed');
  ...
  if (window.ilcSupabase) {
    ...
    document.addEventListener('ilcSupabaseReady', function () { boot(); });
    document.addEventListener('ilcSupabaseError', function () {
      injectNavWidget(); updateNavUI(null); guardPage(null);
    });
